(() => {
  ...
  async function openPanel(book) {
    ...
    els.panelBody.innerHTML = `
      ...
      <div class="panel-actions">
        ...
        <button class="btn btn-ghost" id="panelShareBtn" type="button">Share this book</button>
      </div>
    ...
    document.getElementById('panelShareBtn').addEventListener('click', () => shareBook(book));
  ...
  function shareBook(book) {
    const link = buildLink({ book: book.id, ref: state.username });
    const text = `Check out "${book.title}" by ${book.author} on Smartef Bookshop`;
    if (navigator.share) {
      navigator.share({ title: book.title, text, url: link }).catch(() => {});
    } else {
  ...
  function setUpInvitePanel() {
    ...
    const backdrop = document.getElementById('invitePanelBackdrop');
    const closeBtn = document.getElementById('inviteCloseBtn');
    const linkInput = document.getElementById('inviteLinkInput');
    const copyBtn = document.getElementById('inviteCopyBtn');

    function open() {
      const link = buildLink({ ref: state.username });
      linkInput.value = link;
      const text = encodeURIComponent(`Check out Smartef Bookshop: ${link}`);
      document.getElementById('inviteWhatsapp').href = `https://wa.me/?text=${text}`;
      document.getElementById('inviteTwitter').href = `https://twitter.com/intent/tweet?text=${text}`;
      document.getElementById('inviteFacebook').href = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(link)}`;
