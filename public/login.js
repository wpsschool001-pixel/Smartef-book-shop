(() => {
  ...
  // ---------- capture referral + shared-book links ----------
  const urlParams = new URLSearchParams(window.location.search);
  const refCode = urlParams.get('ref');
  const sharedBookId = urlParams.get('book');
  if (refCode) sessionStorage.setItem('smartef:ref', refCode);
  if (sharedBookId) sessionStorage.setItem('smartef:book', sharedBookId);
  ...
  els.form.addEventListener('submit', async (e) => {
    ...
    if (mode === 'login') {
      ...
      body = { username, password, securityAnswer, email, ref: sessionStorage.getItem('smartef:ref') || '' };
    } else {
    ...
    try {
      ...
      window.location.href = sessionStorage.getItem('smartef:book')
        ? `/?book=${encodeURIComponent(sessionStorage.getItem('smartef:book'))}`
        : '/';
    } catch {
  ...
  setMode(refCode ? 'register' : 'login');
})();
